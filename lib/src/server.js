...
async function render(scene) {
    ...
    if (scene.component.onAppCreated) {
        scene.component.onAppCreated(app)
    }
...
import.meta.hot.send('vshot:error', { error: 'l: '+scenes.length + ' n: ' + scenes[0].componentName })
render(scenes[0])`
}
...
async function takeScreenshots(port, vite, updating) {
    ...
    if (updating) {
        if (fs.existsSync(SCREENSHOTS_PATH + '/current')) fs.rmSync(SCREENSHOTS_PATH + '/current', {
            recursive: true,
            force: true
        })
        if (fs.existsSync(SCREENSHOTS_PATH + '/diff')) fs.rmSync(SCREENSHOTS_PATH + '/diff', {
            recursive: true,
            force: true
        })
    }
    ...
    const takeScreenshot = async (data, client) => {
        const saveTo = (dir) => `${SCREENSHOTS_PATH}/${dir}/${data.componentName}_${data.scene}.png`
        if (updating || !fs.existsSync(saveTo('ref'))) {
            ...
            createIfNotExists(SCREENSHOTS_PATH + '/current')
            ...
            if (diffPixels) {
                createIfNotExists(SCREENSHOTS_PATH + '/diff')
...
export async function createServer(updating) {
    createIfNotExists(SCREENSHOTS_PATH)
    createIfNotExists(SCREENSHOTS_PATH + '/ref')
    ...
    if (relativeFilePaths.length === 0) {
        console.log(warning(`No files found matching pattern: "${PATTERN}"`))
        process.exit()
    }
